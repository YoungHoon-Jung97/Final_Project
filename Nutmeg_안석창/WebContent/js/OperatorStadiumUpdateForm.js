...
$(document).on('click', '.check-dup-btn', function () {
    ...
    const name = $form.find('.stadium-name').val().trim();
    const $msg = $form.find('.name-check-msg');
...
function updateSubmitState($form) {
    const nameChecked = $form.data('nameChecked') === true;

    const start = parseInt($form.find('.time-start').val());
    const end = parseInt($form.find('.time-end').val());

    const isFilled = $form.find('.stadium-name').val().trim() !== '' &&
        $form.find('.post').val().trim() !== '' &&
        $form.find('.address1').val().trim() !== '' &&
        $form.find('.address2').val().trim() !== '' &&
