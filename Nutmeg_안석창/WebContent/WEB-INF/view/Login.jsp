...
<%
	...
	// 이전 페이지 URL을 세션에 저장 (이미 저장된 경우 덮어쓰지 않음)
	if (session.getAttribute("previousPage") == null)
	{
		String previousPage = request.getHeader("Referer"); // 바로 이전 페이지 URL 가져오기
		
		if (previousPage != null && !previousPage.contains("Login.action"))
			// 로그인 페이지로부터 온 경우는 제외
			previousPage = previousPage.replace(".jsp", ".action");
...
<body>
<input type="hidden" name="previousPage" value="<%= session.getAttribute("previousPage") %>">
									...
									<div class="section text-center">
										...
										<form action="LoginCheck.action" method="post" id="loginFormKo">
											...
											<p class="mb-0 mt-4 text-center">
												<a href="Join.action" class="link">회원가입</a>
									...
									<div class="section text-center">
										...
										<form action="LoginCheck.action" method="post" id="loginFormEn">
											...
											<p class="mb-0 mt-4 text-center margin-p">
												<a href="Join.action" class="link">Sign up</a>
												<span class="boundary1">|</span>
