...
<div id="history-container">
    ...
    <div class="pagination" style="margin-top: 20px;">
        <c:if test="${currentPage > 1}">
            <button class="page-btn btn btn-outline-primary" data-page="${currentPage - 1}">이전</button>
        </c:if>

        <c:forEach begin="1" end="${totalPage}" var="i">
            <button class="page-btn btn <c:if test='${i eq currentPage}'>btn-primary</c:if> btn-outline-secondary" data-page="${i}">
                ${i}
            </button>
        </c:forEach>

        <c:if test="${currentPage < totalPage}">
            <button class="page-btn btn btn-outline-primary" data-page="${currentPage + 1}">다음</button>
        </c:if>
    </div>
</div>
...
<h4 class="mb-4">구장 결제 현황</h4>
<div class="match-card">
    <strong>지금까지 총 결제 금액 ${total } 원</strong>
</div>
<c:forEach var="history" items="${fieldResHistoryList}">
    <div class="match-card">
        <div class="stadium-card">
